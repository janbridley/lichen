...
//
//  Restored to the 4547c70 mechanism: the animator drives itself via a Timer
...
final class LichenSaverView: ScreenSaverView {
    ...
    override func layout() {
        super.layout()
        // The backing layer may not exist yet during viewDidMoveToWindow in the
        // appex host; (re)attach here once it does.
        attachIfNeeded()
        animator.updateBounds(bounds)
    }

    /// Idempotently bind the animator to the backing layer once it exists.
